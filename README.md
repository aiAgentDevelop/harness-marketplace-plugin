...
├── benchmarks/                    # Phase 0 A/B pilot (harness effectiveness study)
│   ├── README.md                  # Pilot methodology and how to run
│   ├── tasks/                     # 6 task specs (3 per stack × 2 stacks)
│   ├── reference-projects/        # Seed projects + harness overlays
│   ├── runner/                    # claude -p subprocess runner
│   ├── scorer/                    # Automated + LLM judge scoring
│   └── results/                   # Raw outputs and scored results
...
Empirical validation of whether harnesses generated by `harness-marketplace` improve Claude Code output quality. Inspired by `revfactory/claude-code-harness` but adapted for this plugin's key difference: our harness is dynamically generated per-project rather than a fixed artifact.

**Design**: control (seed only) vs treatment (seed + generated harness overlay), across 2 reference stacks (Next.js+Supabase, FastAPI+Postgres) × 3 difficulty levels (Basic/Advanced/Expert) × N=2 = 24 runs.

**Runner**: `claude -p` subprocess for each run, isolated temp worktree, hook activation preserved in treatment condition.

**Scoring**: automated (file existence, grep patterns, build/test/lint if feasible, hook block count) + LLM judge (blind 4-dimension rubric: code quality, completeness, edge cases, security).
