...
## interview.md (Phase -1 → Phase 0+1+2+3 — interview 모드 시만)

```markdown
# Handoff: project-interview → project-plan

## 작업 개요 / Task Info
- **서비스 설명**: {service_description}
- **인터뷰 모델**: {interview_model}  # sonnet | opus
- **팀**: {team_name}

## 인터뷰 요약 / Interview Summary
- 라운드: {rounds_completed}회
- 질문 수: {total_questions}개
- 답변 수: {total_answers}개
- 구현 명확도: {clarity_percentage}%

### 차원별 명확도 / Clarity Breakdown
| 차원 | 명확도 |
|------|--------|
| vision | {pct}% |
| users | {pct}% |
...
## PRD 참조 / PRD Reference
- **경로**: .claude/skills/project-harness/prd/service-prd.md
- **섹션**: {sections 목록}
- **분량**: {word_count}자

## 생성된 에이전트 / Created Agents
| 에이전트 | 경로 | 출처 | 모델 |
|----------|------|------|------|
| {agent_id} | agents/{agent_id}.md | {deep_research|interview_context} | {model} |

## 활성화된 기존 에이전트 / Activated Existing Agents
- {agent_id}: {활성화 사유}

## 팀 구성 / Team Composition
| 역할 | 에이전트 | 참여 단계 | 출처 |
|------|----------|----------|------|
| {role} | {agent_id} | {plan, implement, verify} | {existing|created} |
...
- Phase 0+1+2+3 (project-plan) 에서 PRD + 인터뷰 결과를 컨텍스트로 활용
- 데이터 위치:
  - Notepad 키: `project-interview-result`
  - JSON 결과: `state/results/interview.json` (schemas.md 의 InterviewResult 스키마)
  - PRD 파일: `.claude/skills/project-harness/prd/service-prd.md`
```
...
├── interview.md    # Phase -1 결과 → Phase 0+1+2+3 입력 (interview 모드 시)
