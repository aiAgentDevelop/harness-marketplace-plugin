...
| 모드 | 적합한 경우 | 동작 방식 |
|------|-----------|----------|
| **Deep Interview** | 새 프로젝트, 기술 선택이 불확실할 때 | AI가 3-5개 질문으로 프로젝트 기획을 파악 후 전체 아키텍처를 추천. 검토 후 확인. |
| **Manual** | 이미 기술 스택을 알고 있을 때 | 프로젝트 유형, 언어, DB, 플랫폼, 기술 스택을 하나씩 직접 선택. 선택적 프로젝트 설명으로 AI 추천 라벨 활성화. |
| **Auto-Detect** | 기존 프로젝트에 harness 추가할 때 | AI가 프로젝트 파일(package.json, 설정 파일, 코드 구조)을 스캔하여 사용 중인 아키텍처와 기술 스택을 자동 감지. |
